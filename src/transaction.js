 ...
 */
 function Transaction(bitcoins, highlight, currency, currencyName) {
	...
	const strADON = bitcoins.toFixed(2);
	let strADONDisplay = "";
	
	if (strADON === "0.00") {
	    strADONDisplay = "&lt;0.01<span class='bitcoinsymbol'>ADON</span>";
	} else {
	    strADONDisplay = strADON + "<span class='bitcoinsymbol'>ADON</span>" ;
	}

	if (!highlight) {
		this.addText(strADONDisplay);
	} else {
		this.addText('<span style="color: #ffb600;">' + strADONDisplay + '</span><br /><span style="color: cyan;">Donation</span><br /><span style="color: lime;">Thanks!</span>');
	}
