...
function getTx(strHash) {
	...
	txReq.open('GET', 'https://mainnet.adonis.network/api/v2/tx/' + strHash);
	txReq.send();
}
...
function checkForNewTxOrBlock(fBlockOnly = false, strBlockHash = strLastBlock) {
	...
	if (!fBlockOnly) {
		...
		mempoolReq.open('GET', 'https://mainnet.adonis.network/api/v2/mempool/');
		mempoolReq.send();
	}
	...
	if (!strLastBlock) {
		...
		initBlockReq.open('GET', 'https://mainnet.adonis.network/api/v2/api');
		initBlockReq.send();
	} else {
		...
		blockReq.open('GET', 'https://mainnet.adonis.network/api/v2/block/' + strBlockHash);
		blockReq.send();
	}
}
